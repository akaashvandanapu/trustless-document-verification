...
export default function Header() {
  ...
  // Hide navigation and account details on verification pages (links sent)
  const isVerificationPage = pathname?.startsWith("/academic/") || 
                             pathname?.startsWith("/pan/") || 
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 transition-colors">
      <div className="max-w-7xl mx-auto flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        {/* Navigation - only show on main pages, not verification detail pages */}
        {!isVerificationPage && (
        ...
        {/* Actions - hide on verification pages */}
        {!isVerificationPage && (
